...
plt.show()
```
위 코드를 통해 잔차를 비교할 수 있는 그래프를 생성할 수 있다. 이 때 진차는 (실제값 - 예측값)이며, 모델의 예측 값이 실제 값에서 얼마나 벗어났는지를 알 수 있다. 위 코드는 잔차의 빈도 그래프와 잔차의 산점도 그래프를 생성한다.

![Residual Analysis table1](./blog-code_files/blog-code_3_1.png)

위 그래프는 잔차의 빈도를 나타내는 그래프이다. 그래프 내에서 잔차값이 낮은 빈도가 높을수록 모델의 예측이 정확하다.

위 그래프를 보면 잔차가 0과 2 사이일 때 매우 잦은 빈도가 발생한 것을 나타내고 있다. 위 잔차 정의를 보았을 때 이 모델은 예측값과 실제값이 유사한 빈도가 높다는 것을 나태내었으므로 모델의 예측 정도를 신뢰할 수 있다고 볼 수 있다.

![Residual Analysis table2](./blog-code_files/blog-code_3_2.png)

위 그래프는 잔차 산점도 그래프를 나타낸다. 잔차 산점도는 모델의 예측 값과 잔차를 비교하여, 모델이 데이터를 적합하게 학습하고 있는지 확인하는 데 사용된다. 이 그래프가 나타내는 잔차의 패턴과 분포를 해석하는 방법으론 대표적으로 4가지가 있다.
...
위에서 언급한 딥러닝 모델의 정확도를 판단하는 그래프들의 해석을 정리하면 다음과 같다. 

<ins>1번 그래프(학습과 검증 손실 그래프)</ins>의 경우 validation loss 그래프와 training loss 가 수렴하며 우하향하는 형태를 띄어야 하는데 validation loss 가 1. training loss랑 수렴하지 않고 2. 우하향하지 않고 증감을 반복하는 추세를 보인다. 

<ins>2번 그래프(평균의 절대오차(MAE) 그래프)</ins>의 경우 validation loss 그래프의 흔들리는 정도가 큰 것을 보아 과적합이 의심된다. 모델이 균형잡힌 학습을 하고 있지 않다는 것을 의미한다.

<ins>3번 그래프(예측값과 실제값에 대한 그래프)</ins>는 actual G3 점수가 0.0, 7.5 일 때의 값을 제외하면 나머지 점수 대에서의 값들은 예측값에 근접한 결과를 보인다.

<ins>4번 그래프(잔차 빈도 그래프)</ins>는 3번 그래프의 잔차 빈도를 나타낸 값이므로 3번과 동일한 결과를 나타낸다. 즉, 성적이 0.0, 7.5인 경우를 제외하면 낮은 잔차값(0에 가까운) 빈도가 높으므로 실제값과 어울리는 예측을 하고 있다고 볼 수 있다. 

<ins>5번 그래프(잔차 산점도 그래프)</ins>는 제대로 학습이 이루어졌다면 점들이 패턴 없이 무작위로 분포해야한다. 그러나 본 그래프는 잔차의 양의 영역으로 치우쳤으므로 모델이 편향된 예측을 하고 있음을 알 수 있다. 

<ins>6번 그래프(특성 중요도 그래프)</ins>는 G2 의 중요도를 과하게 높게 평가하는 것으로 보아 모델이 편향된 예측을 하고 있음을 알 수 있다. 

위 해석들을 바탕으로 해당 학습 모델은 편향된 예측을 하는 모델임을 알 수 있고, 이에 대한 원인을 찾아보았다. 
> 데이터가 편향되어 모델이 특정 패턴을 과도하게 학습하여 과적합으로 이어졌을 수 있다. 특히나 특성 중요도 그래프를 살펴보면 G2 데이터를 과도하게 학습하였다. 모델이 학습 데이터에 과적합 되면 그 데이터의 특정한 편향을 반영하는 예측을 할 수 있다. 즉, 편향된 데이터는 모델의 과적합을 유도했고, 과적합된 모델은 다시 편향된 예측을 하게 된다.
> 추가적으로 과적합 발생 원인을 추론해보았다. (과적합: 모델이 학습 데이터에는 높은 성능을 보이지만 새로운 테스트 데이터에서는 성능이 저하되는 현상)
...
이 문제를 해결하기 위해선 앞서 말했듯이 더 많은 데이터를 확보하거나 모델을 간소화하면 과적합을 해결할 수 있다.
   ...
   과적합은 모델이 학습 데이터에는 높은 성능을 보이지만 새로운 테스트 데이터에는 성능이 저하되는 현상이다. 
   과적합 발생 원인을 추론해보았다. 
  
1. 데이터 수가 적다
   위 딥러닝 모델의 경우 test_size를 0.2로 설정해 데이터의 20%를 테스트 데이터로, 80%를 학습 데이터로 설정하였다. 
   kaggle 데이터셋은 총 33개 column, 1044개 row로 이뤄져있다. 즉 약 209개의 데이터가 테스트 데이터로, 약 835개의 데이터가 학습 데이터로 사용되었다.
   우선 데이터셋 자체가 작으므로 학습에 사용되는 데이터가 부족해 모델이 데이터에 과도하게 적합할 가능성이 있다. 또한 테스트 데이터 역시 적기 때문에 모델의 성능이 신뢰할 만하지 않을 수 있다.



[과적합 발생 이유]
1. 드롭아웃 비율 낮음
20%는 비교적 보수적. 30% 50% 시도
2. 모델의 복잡도가 너무 높음
층의 수나 뉴런 수가 많아 학습 데이터에 과도하게 적합 했을 수 있음
뉴런의 수를 줄이거나 층의 수를 줄여 복잡도 낮춤
3. 학습 데이터가 부족함


하이퍼파라미터인 뉴런 수 unit / 층 수 number of layers / 활성화 함수 activation function

하이퍼파라미터는 모델의 성능과 과적합 방지에 영향을 미치며 상황에 따라 적절히 바꿔야함.

너무 많은 층을 사용하면 과적합, 너무 적으면 과소적합 발생 가능
...
이를 해결하기 위해 
1. 충분히 큰 데이터셋을 확보한다.
   데이터셋이 적다면, 테스트 세트를 더 적게 설정하거나 교차 검증을 활용하는 것이 좋다.
2. 교차 검증을 사용한다
  k-폴드 교차 검증을 사용해 학습 및 평가 데이터의 효율성을 극대화할 수 있다.
3. 정규화 및 드롭아웃
    드롭아웃은 과적합 방지에 효과적
    학습 데이터가 적을 수록 드롭 아웃 비율을 높이는 것이 도움됨
4. 적절한 모델 복잡성 유지
    모델의 복잡도가 학습 데이터 크기에 비해 지나치게 크면 과적합이 발생하기 쉽다. 우리 모델의 경우 column이 33개로, feature이 상당히 많다. 이에 따라 과적합이 발생하여 G2의 중요도가 필요 이상으로 높아졌다.
