...
<ins>1번 그래프(training and validation loss)</ins>의 경우 validation loss 그래프와 training loss 가 수렴하며 우하향하는 형태를 띄어야 하는데 validation loss 가 1. training loss랑 수렴하지 않고 2. 우하향하지 않고 증감을 반복하는 추세를 보인다. 

<ins>2번 그래프(training and validation MAE)</ins>의 경우 validation loss 그래프의 흔들리는 정도가 큰 것을 보아 과적합이 의심된다. 모델이 균형잡힌 학습을 하고 있지 않다는 것을 의미한다.

<ins>3번 그래프</ins>는 actual G3 점수가 0.0, 7.5 일 때의 값을 제외하면 나머지 점수 대에서의 값들은 예측값에 근접한 결과를 보인다.

<ins>4번 그래프</ins>는 3번 그래프의 잔차 빈도를 나타낸 값이므로 3번과 동일한 결과를 나타낸다. 즉, 성적이 0.0, 7.5인 경우를 제외하면 낮은 잔차값(0에 가까운) 빈도가 높으므로 실제값과 어울리는 예측을 하고 있다고 볼 수 있다. 
