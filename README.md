...
2. 드롭아웃 비율이 낮다
    - 드롭아웃은 과적합 방지에 효과적이다. 20%는 비교적 낮은 수치이다. 


3. 모델의 복잡도가 높다
    - 층의 수나 뉴런 수가 많아 학습 데이터에 과도하게 적합 했을 수 있다. 위 모델에서 설정한 층의 수는 3개이고 뉴런의 수는 128, 64, 32개로 보편적으로 너무 많은 수는 아니지만, 데이터셋이 적다는 것을 고려했을 때 복잡도에 영향을 미쳤을 가능성이 있다. 
...
2. 교차 검증을 사용한다. 
    - k-폴드 교차 검증을 사용해 학습 및 평가 데이터의 효율성을 극대화할 수 있다.
      - k-폴드 교차 검증: 데이터셋을 균등하게 K개의 부분으로 나누어 학습과 검증을 반복적으로 수행하는 방법

3. 드롭아웃 수치를 조절한다.
    - 드롭아웃은 과적합 방지에 효과적이다. 학습 데이터가 적을수록 드롭 아웃 비율을 높이는 것이 도움된다.

4. 적절한 모델 복잡성 유지
    - 모델의 복잡도가 학습 데이터 크기에 비해 지나치게 크면 과적합이 발생하기 쉽다. 우리 모델의 경우 데이터셋 크기에 비해 column이 33개로, feature이 상당히 많다. 이에 따라 과적합이 발생하여 G2의 중요도가 필요 이상으로 높아졌다.
